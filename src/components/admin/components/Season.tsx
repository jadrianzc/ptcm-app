...
export const Season = () => {
	...
	useEffect(() => {
		getMatchDay(seasons[0]?.id)
			...
			.catch((err) => console.log(err));
	}, [seasons, setSeason, setMatchDay, setUpcomingDates, setCompletedDates]);
	...
	const addMatchDay = async () => {
		...
		const totalSeasonMatchs = matchDays.filter((match) => !match.name.endsWith('(b)')).length;
		...
		const { data } = await localApi.post('/admin/setNewMatchDay', dataMatchDay);
		console.log(data);
	};
											...
											onClick={() => {
												getMatchDay(seasons[index]?.id)
													.then((resp) => {
														setMatchDay(resp.respMatchDays.data);
														setUpcomingDates(
															resp.respMatchDays.upcomingDates,
														);
														setCompletedDates(
															resp.respMatchDays.completed,
														);
													})
													.catch((err) => console.log(err));
											}}
