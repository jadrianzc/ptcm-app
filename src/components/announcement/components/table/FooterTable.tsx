...
import { IMatches, IResponseGroup } from '../../interfaces';
...
export const FooterTable = () => {
	...
	const createMatchesForGroup = async () => {
		try {
			...
			const matchesCreate = groups.map(({ idGroup, idCancha, groups }): IMatches[] => {
				const matches: IMatches[] = generateMatches(groups, idGroup, idCancha);
